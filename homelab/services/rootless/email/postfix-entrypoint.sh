...
{{ homelab.priv_main_email }} [smtp.gmail.com]:587
hireme@ebra.dev [smtp.gmail.com]:587
me@ebra.dev [smtp.gmail.com]:587
sayhi@ebra.dev [smtp.gmail.com]:587
{{ homelab.priv_qa_email }} [smtp.gmail.com]:587
auth@ebra.dev [smtp.gmail.com]:587
vaultwarden@ebra.dev [smtp.gmail.com]:587
{{ homelab.priv_icloud_email }} [smtp.mail.me.com]:587
EOF

cat > /etc/postfix/allowed_senders <<'EOF'
{{ homelab.priv_main_email }} OK
hireme@ebra.dev OK
me@ebra.dev OK
sayhi@ebra.dev OK
{{ homelab.priv_qa_email }} OK
auth@ebra.dev OK
vaultwarden@ebra.dev OK
{{ homelab.priv_icloud_email }} OK
EOF

umask 077
{
	printf '%s %s:%s\n' '{{ homelab.priv_main_email }}' '{{ homelab.priv_main_email }}' "$GMAIL_MAIN_APP_PASSWORD"
	printf '%s %s:%s\n' 'hireme@ebra.dev' '{{ homelab.priv_main_email }}' "$GMAIL_MAIN_APP_PASSWORD"
	printf '%s %s:%s\n' 'me@ebra.dev' '{{ homelab.priv_main_email }}' "$GMAIL_MAIN_APP_PASSWORD"
	printf '%s %s:%s\n' 'sayhi@ebra.dev' '{{ homelab.priv_main_email }}' "$GMAIL_MAIN_APP_PASSWORD"
	printf '%s %s:%s\n' '{{ homelab.priv_qa_email }}' '{{ homelab.priv_qa_email }}' "$GMAIL_QA_APP_PASSWORD"
	printf '%s %s:%s\n' 'auth@ebra.dev' '{{ homelab.priv_qa_email }}' "$GMAIL_QA_APP_PASSWORD"
	printf '%s %s:%s\n' 'vaultwarden@ebra.dev' '{{ homelab.priv_qa_email }}' "$GMAIL_QA_APP_PASSWORD"
	printf '%s %s:%s\n' '{{ homelab.priv_icloud_email }}' '{{ homelab.priv_icloud_email }}' "$ICLOUD_APP_PASSWORD"
	printf '%s %s:%s\n' '[smtp.gmail.com]:587' '{{ homelab.priv_main_email }}' "$GMAIL_MAIN_APP_PASSWORD"
	printf '%s %s:%s\n' '[smtp.mail.me.com]:587' '{{ homelab.priv_icloud_email }}' "$ICLOUD_APP_PASSWORD"
